...
private:
    ...
    int main() {
...

      {
        ...
        // But now, let's sleep for 8 more seconds and submit a new job, that lasts for 10 seconds
        // In this case, after 10th second, the host can works at a speed of 0 flops/seconds, so we
        // want to check if simgrid will throw any error and what error type will simgrid throw
        wrench::S4U_Simulation::sleep(8);
        // Submit the job for execution, I should get a job some kind of error
        try {
        ...
        //Now sleep for 8 seconds, so total seconds until now is 17, so probably at this time, when we wake up
